...
### Dependency Review and Software Composition Analysis (SCA)
...
- **Deeper Dive:**
    ...
    - **Tools:** CodeQL, ESLint, Biome with security plugins (JS/TS), etc.
...
- **Deeper Dive:**
    ...
    - **Cache Keys:** Use a unique key based on file hashes (e.g., `hashFiles('**/pnpm-lock.yaml')`, `hashFiles('**/requirements.txt')`) to invalidate the cache only when dependencies change.
...
- name: Cache Node.js modules
  ...
  with:
    ...
    key: ${{ runner.os }}-node-${{ hashFiles('**/pnpm-lock.yaml') }}-${{ github.run_id }}
    restore-keys: |
...
### Artifacts for Inter-Job and Inter-Workflow Communication
...
- **Guidance for Copilot:**
    ...
    - Use appropriate language-specific test runners and frameworks (Vitest, React Testing Library, etc.).
...
- **Deeper Dive:**
    ...
    - **Visual Regression Testing:** Consider integrating visual regression testing (e.g. Percy, Playwright) to catch UI discrepancies.
...
- **Guidance for Copilot:**
    ...
    - Implement manual approval steps for production deployments, potentially integrating with external ITSM (IT Service Management) or change management systems.
...
### Deployment Types (Beyond Basic Rolling Update)
...
### Workflow Not Triggering or Jobs/Steps Skipping Unexpectedly
...
### Permissions Errors (`Resource not accessible by integration`, `Permission denied`)
...
### Caching Issues (`Cache not found`, `Cache miss`, `Cache creation failed`)
...
### Long Running Workflows or Timeouts
...
### Flaky Tests in CI (`Random failures`, `Passes locally, fails in CI`)
...
### Deployment Failures (Application Not Working After Deploy)
